...
UA='Mozilla/5.0 (compatible; OptionsAmericaMigration/3.0)'
    ...
    for i,row in enumerate(rows,1):
        ...
        if row['type']=='video':
            try:
                ...
                row['vimeoId']=None
            time.sleep(.15)
    if rows and rows[-1]['title'].lower().endswith('text lesson'):
        rows[-1]['type']='text'; rows[-1]['duration']='Text lesson'; rows[-1]['vimeoId']=None
    data={'course':{'id':49881,'slug':'options-trading-level-3-buying-put-option-strategy','title':'Options Trading Course Level 3: Buying Put Option','level':'Beginner','sourceUrl':COURSE_URL},'lessons':rows}
