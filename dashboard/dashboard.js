...
function pageButtons(page,total){const wanted=[1,total,page-1,page,page+1].filter(n=>n>=1&&n<=total),pages=[...new Set(wanted)].sort((a,b)=>a-b);let previous=0;return pages.map(n=>{const gap=n-previous>1?'<span class="pagination-gap" aria-hidden="true">…</span>':'';previous=n;return`${gap}<button type="button" data-page="${n}" ${n===page?'class="active" aria-current="page"':''}>${n}</button>`}).join('')}
function renderCards(activity){const filtered=activity.filter(c=>currentFilter==='completed'?c.percent===100:currentFilter==='active'?c.started&&c.percent<100:c.started),totalPages=Math.ceil(filtered.length/PAGE_SIZE);currentPage=totalPages?Math.min(Math.max(currentPage,1),totalPages):1;const visible=filtered.slice((currentPage-1)*PAGE_SIZE,currentPage*PAGE_SIZE);$('#learningGrid').innerHTML=visible.map(course=>`<article class="learning-card">${courseImage(course,'learning-card-image')}<div class="learning-card-body"><span class="course-level">${course.level}</span><h3>${course.title}</h3><div class="progress-track"><i style="width:${formatPercent(course.percent)}%"></i></div><div class="progress-label"><span>${course.done} / ${course.total} lessons</span><strong>${formatPercent(course.percent)}%</strong></div><div class="learning-card-foot"><span>${course.percent===100?'Course completed':'Lesson '+course.next+' is next'}</span><a href="${playerLink(course)}">${course.percent===100?'Review':'Continue'} →</a></div></div></article>`).join('');$('#learningGrid').hidden=!filtered.length;$('#emptyLearning').hidden=!!filtered.length;$('#emptyLearning h3').textContent=activity.some(c=>c.started)?(currentFilter==='completed'?'No completed courses yet':'No courses in this view'):'Your learning journey starts here';const pagination=$('#coursePagination');pagination.hidden=totalPages<=1;pagination.innerHTML=totalPages>1?`<button type="button" class="pagination-direction" data-page="${currentPage-1}" ${currentPage===1?'disabled':''}>← Previous</button>${pageButtons(currentPage,totalPages)}<button type="button" class="pagination-direction" data-page="${currentPage+1}" ${currentPage===totalPages?'disabled':''}>Next →</button>`:''}
...
document.querySelectorAll('[data-filter]').forEach(button=>button.addEventListener('click',()=>{currentFilter=button.dataset.filter;currentPage=1;document.querySelectorAll('[data-filter]').forEach(x=>x.classList.toggle('active',x===button));renderDashboard()}));
$('#coursePagination').addEventListener('click',event=>{const button=event.target.closest('button[data-page]');if(!button||button.disabled)return;currentPage=Math.max(1,Number(button.dataset.page)||1);renderDashboard();document.querySelector('.learning-section')?.scrollIntoView({behavior:'smooth',block:'start'})});
